...
var CheckInList = require('./CheckInList');
...
var CheckInDB = React.createClass({
    getInitialState(){
    ...
    populateDB(tx) {
        ...
        tx.executeSql('DROP TABLE IF EXISTS CheckInPersons');
        ...
        /*tx.executeSql('CREATE TABLE IF NOT EXISTS CheckInPersons( '
            + 'id VARCHAR(55) PRIMARY KEY NOT NULL, '
            + 'name VARCHAR(30),'
            + 'week VARCHAR(30)'
            + 'date VARCHAR(30)); ', [], this.successCB, this.errorCB);*/
        ...
        tx.executeSql('INSERT INTO Departments (name) VALUES ("成真");', []);
        ...
        tx.executeSql('INSERT INTO Offices (name, longtitude, latitude) VALUES ("Beijing", 116, 39.);', []);
        ...
        tx.executeSql('INSERT INTO Employees (name, office, department) VALUES ("Guanganmen", 5, 6);', []);

        //[otx.executeSql('INSERT INTO CheckInPersons (id, name, week, date) VALUES ("8034aa76d7a84c71", "测试账号二", "星期三", "2016-08-24");', []);
    ...
    queryEmployees(tx) {
        ...
        //tx.executeSql('SELECT * from CheckInPersons', [],this.queryEmployeesSuccess(), this.errorCB);
    },
    ...
    queryEmployeesSuccess(tx,results) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            this.state.progress.push(`Empl Name: ${row.name}, Week: ${row.week}, Date: ${row.date}`);
        }
        this.setState(this.state);
    },
    ...
    queryCheckinList(){
        ...
        db.transaction(function() {
            this.executeSql('SELECT * FROM CheckInPersons', [],

                that.queryCheckinListSuccess, that.errorCB);
        }, that.errorCB, function () {
            console.log("Database CheckInPersons query ending ...");
            db.close(that.closeCB, that.errorCB);
        })
    },

    queryCheckinListSuccess(tx, results){
        this.state.progress.push("签到表 Query completed");
        this.setState(this.state);
        var len = results.rows.length;
        for (let i = 0; i < len; i++) {
            let row = results.rows.item(i);
            this.state.progress.push(`签到人: ${row.name}, 签到时间: ${row.week} ${' ' + row.time}`);
        }
        this.setState(this.state);
    },
    ...
    render(){
        ...
        return (<View style={styles.mainContainer}>
            <View style={styles.toolbar}>
                ...
                <Text style={styles.toolbarButton} onPress={this.queryCheckinList}>
                    签到表
                </Text>
